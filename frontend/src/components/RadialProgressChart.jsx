...
const MultiDonutChart = ({ data, size = 420, strokeWidth = 28, chartTitle, chartSubtitle }) => {
  const [tooltip, setTooltip] = useState(null);
  const base = size / 2;
  const gap = 8;
  const radii = NUTRIENTS.map((_, i) =>
    base - (strokeWidth / 2) - i * (strokeWidth + gap)
  );
  const getTooltipPos = (angle, i) => {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: size + 220, marginTop: 56 }}>
      <div style={{ flex: 1 }}>
        {chartTitle && <h2 style={{ textAlign: 'center', fontWeight: 700, fontSize: 26, marginBottom: 24 }}>{chartTitle}</h2>}
        {chartSubtitle && <div style={{ textAlign: 'center', color: '#888', fontSize: 16, marginBottom: 24 }}>{chartSubtitle}</div>}
        ...
        {tooltip && (
          <div style={{
            ...
            <div style={{ fontWeight: 700, fontSize: 18, marginBottom: 6 }}>{tooltip.nutrient.label}</div>
      ...
      {/* 범례 */}
      <div style={{ marginLeft: 40, display: 'flex', flexDirection: 'column', gap: 16, fontSize: 17, fontWeight: 600 }}>
        {NUTRIENTS.map(n => (
          <div key={n.key} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ display: 'inline-block', width: 16, height: 16, borderRadius: 8, background: n.color }} />
